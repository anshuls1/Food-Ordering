...
from .models import Category, RegularPizza, SicilianPizza, Toppings, Sub, Pasta, Salad, Drinks,Donuts,Burger,Noodles, UserOrder, SavedCarts
...
def donuts(request):
    if request.user.is_authenticated:
        return render(request, "orders/donuts.html", context = {"dishes":Donuts.objects.all})
    else:
        return redirect("orders:login")

def noodles(request):
    if request.user.is_authenticated:
        return render(request, "orders/noodles.html", context = {"dishes":Noodles.objects.all})
    else:
        return redirect("orders:login")

def burger(request):
    if request.user.is_authenticated:
        return render(request, "orders/burger.html", context = {"dishes":Burger.objects.all})
    else:
        return redirect("orders:login")
